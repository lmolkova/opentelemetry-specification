...
| OTEL_SERVICE_NAME        | Sets the value of the [`service.name`](/specification/resource/semantic_conventions/README.md#servicename) resource attribute | | If `service.name` is also provided in `OTEL_RESOURCE_ATTRIBUTES`, then `OTEL_SERVICE_NAME` takes precedence. |
