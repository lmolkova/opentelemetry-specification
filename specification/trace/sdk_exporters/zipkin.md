...
|3|server.nat.address|[OpenTelemetry adopted attribute for remote socket hostname of the peer.](../semantic_conventions/span-general.md#general-network-connection-attributes)|
|4|server.nat.ip & server.nat.port|[OpenTelemetry adopted attribute for remote socket address of the peer.](../semantic_conventions/span-general.md#general-network-connection-attributes)|
...
* `server.nat.address` and `server.nat.ip` can be used by themselves as `remoteEndpoint` but should be combined
  with `server.nat.port` if it is also present.
